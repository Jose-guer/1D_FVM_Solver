function Roe = ROEflux(UL,UR,AL,AR,gamma)
% This function computes the Roe flux function for flows in which gamma is
...
uL = UL(2)./UL(1);
EL = UL(3)./UL(1);
pL = (gamma-1)/AL * ( UL(3) - 0.5*UL(2)^2/UL(1) );
HL = EL + pL/rhoL;
...
uR = UR(2)./UR(1);
ER = UR(3)./UR(1);
pR = (gamma-1)/AR * ( UR(3) - 0.5*UR(2)^2/UR(1) );
HR = ER + pR/rhoR;
